...
object ApplicationBuild extends Build {
  ...
  lazy val root = Project("edu_anabim", file("."))
    ...
    .settings(Seq(
      ...
      libraryDependencies ++= Seq(
        ...
        play_silhouette, scala_guice, ficus
...
//      resourceDirectory in Assets := (sourceDirectory in Compile).value / "assets"
  ...
  lazy val anabim = project("anabim", Seq(common, db, memo, hub, user, counter, image)).settings(
    libraryDependencies ++= provided(play.api, reactivemongo.driver)
  )

  lazy val image = project("image", Seq(common, db, memo, hub, user, counter)).settings(
    libraryDependencies ++= provided(play.api, reactivemongo.driver)
  )
