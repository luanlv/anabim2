...
trait AssetHelper { self: I18nHelper =>
  ...
  def csDev(name: String) = staticUrl + "/file/" + name + "?v=" + scala.util.Random.nextInt(10000)
  ...
  def categories = Json.toJson(CateRepo.allCates().await)
  def softs = Json.toJson(SoftwareRepo.allSofs().await)
  def price = Json.toJson(PriceRepo.get.await)
}
